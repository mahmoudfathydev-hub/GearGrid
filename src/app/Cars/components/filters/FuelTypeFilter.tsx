...
export default function FuelTypeFilter({ value, onChange, options = [] }: FuelTypeFilterProps) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-40">
        <DropdownMenuItem onClick={() => onChange("")}>
          <span className="text-gray-500">All Fuels</span>
        </DropdownMenuItem>
        {options.map((fuel) => (
          <DropdownMenuItem
            key={fuel}
            onClick={() => onChange(fuel)}
            className={value?.toLowerCase() === fuel.toLowerCase() ? "bg-gray-100 dark:bg-gray-700" : ""}
          >
            {fuel}
          </DropdownMenuItem>
        ))}
