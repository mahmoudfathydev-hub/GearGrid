...
export function AIChatBot({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: comparisonData?.comparison
  ...
  const generateContext = () => {
    if (!comparisonData || !cars) {
      return "No cars are currently being compared.";
    }
  ...
  const callHuggingFaceAPI = async (userMessage: string) => {
    try {
      ...
      const prompt = `You are a knowledgeable car comparison expert. Use the following car comparison data to answer the user's question accurately and helpfully.

${context}

User Question: ${userMessage}

Provide a detailed, helpful response based on the car data above. If the user asks for recommendations, consider their specific needs and the car specifications. Be conversational and friendly.`;
      ...
      return "I'm having trouble connecting to my AI services right now. Please try again later or ask me a simpler question about the cars you're comparing.";
    }
  };
  ...
  return (
    <>
      {isOpen && (
        <Card className="fixed bottom-4 right-4 w-96 h-[600px] flex flex-col shadow-lg border-2 border-blue-200 z-40">
            ...
            <div className="p-4 border-t">
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="Ask about the cars you're comparing..."
                  disabled={isLoading}
                  className="flex-1"
                />
                <Button
